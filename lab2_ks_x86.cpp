...
int main()// Oo0LlIi
{
    ...
    for(Capacity = 2000; Capacity <= 100000; Capacity += 2000){
        ...
        for(int i = 0; i < repeat; i++){
            ...
            elapsed_time += (t2.QuadPart - t1.QuadPart);
        }
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "Basic Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "Basic Update Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "Basic Update Temp Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "SSE Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "SSE Update Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;

        // SSE提前设定临时变量存储Value[i]和Weight[i]避免运行中频繁访问数组
        for(int i = 0; i < repeat; i++){
            Zero(Solution_previous, Capacity_add_one);
            QueryPerformanceCounter(&t1);// 计时开始
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "SSE Update Temp Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "SSE Update Temp Align Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        for(int i = 0; i < repeat; i++){
            Zero(Solution_previous, Capacity_add_one);
            QueryPerformanceCounter(&t1);// 计时开始
            Knapsack_AVX(Weight, Value, Nsize, Capacity, Solution_previous, Solution_current);
            QueryPerformanceCounter(&t2);// 计时结束
            elapsed_time += (t2.QuadPart - t1.QuadPart);
        }
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "AVX Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        for(int i = 0; i < repeat; i++){
            Zero(Solution_previous, Capacity_add_one);
            QueryPerformanceCounter(&t1);// 计时开始
            Knapsack_AVX_update(Weight, Value, Nsize, Capacity, Solution_previous, Solution_current);
            QueryPerformanceCounter(&t2);// 计时结束
            elapsed_time += (t2.QuadPart - t1.QuadPart);
        }
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "AVX Update Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;

        // AVX提前设定临时变量存储Value[i]和Weight[i]避免运行中频繁访问数组
        for(int i = 0; i < repeat; i++){
            Zero(Solution_previous, Capacity_add_one);
            QueryPerformanceCounter(&t1);// 计时开始
            Knapsack_AVX_update_temp(Weight, Value, Nsize, Capacity, Solution_previous, Solution_current);
            QueryPerformanceCounter(&t2);// 计时结束
            elapsed_time += (t2.QuadPart - t1.QuadPart);
        }
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "AVX Update Temp Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
        ...
        for(int i = 0; i < repeat; i++){
            Zero(Solution_previous, Capacity_add_one);
            QueryPerformanceCounter(&t1);// 计时开始
            Knapsack_AVX_update_temp_align(Weight, Value, Nsize, Capacity, Solution_previous, Solution_current);
            QueryPerformanceCounter(&t2);// 计时结束
            elapsed_time += (t2.QuadPart - t1.QuadPart);
        }
        elapsed_time = elapsed_time * 1000.0 / frequency.QuadPart / repeat;
        std::cout << "AVX Update Temp Align Time: " << elapsed_time << "ms" << std::endl;
        elapsed_time = 0;
